...
class WisdomBadgesListKeyword(BaseAPIView):
    swagger_query_params = [
        SwaggerQueryParam("keyword", True, schema=coreschema.String()),
    ...
    def _get(self, request):
        ...
        for key in keys:
            ...
            filter_args |= Q(
                knowledge_badges_wisdom_badges__criteria_narrative__icontains=key
            )
            filter_args |= Q(
                knowledge_badges_wisdom_badges__criteria_knowledge_badges__name__icontains=key
            )
        queryset = (
