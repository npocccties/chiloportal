...
class Command(BaseCommand):
    ...
    def update_wisdom_badge(self, json, portal_category):
        ...
        if wisdom_badge_set.exists():
            ...
            logger.debug('--- insert ---')
            wisdom_badge = WisdomBadges.objects.create(badge_class_id=id, name=name, description=description, image_id=image_fname,
             image_author=image_author, version=version, issuer=issuer, alignments_targeturl=target_url, alignments_targetname=target_name,
    ...
    def create_knowledge_badges(self, alignments, wisdom_badge):
        for alignment in alignments:
            ...
            if '/badges/badge_json' in target_url:
                ...
                image_path = self.download_file(image_id)
                knowledge_badge, criteria_list = self.create_knowledge_badge(knowledge_json, wisdom_badge, image_path)
                criteria_sort_key = 1
    ...
    def create_criterias(self, knowledge_badge, criteria_list, criteria_sort_key):
        for criteria in criteria_list:
            if len(criteria) < 2:
                continue
            self.create_criteria(criteria[0], criteria[1], knowledge_badge, criteria_sort_key)
            criteria_sort_key += 1
    ...
    def create_knowledge_badge(self, json, wisdom_badge, image_path):
        ...
        knowledge_badge = KnowledgeBadges.objects.create(badge_class_id=id, name=name, description=description, criteria_narrative=criteria_narrative,
            image_id = os.path.basename(image_path), image_author=image_author, version=version, issuer=issuer, wisdom_badges=wisdom_badge)
    ...
    def create_criteria(self, type, name, knowledge_badge, sort_key):
        criteria = Criteria.objects.create(type=type, name=name, knowledge_badges=knowledge_badge, sort_key=sort_key)
