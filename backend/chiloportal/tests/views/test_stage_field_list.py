...
class StageFieldListTests(BaseAPIViewTests):
    def test_stage_field_list_200(self):
        ...
        response = self.request_normal(factory, view, self.stage_field_list_url, {'stage_id': self.stg1.id})
        self.assertEqual(len(response.data), 5)
        array = response.data
        field1 = array[0].get('field1')
    ...
    def test_stage_field_list_400_invalid_value(self):
        factory = APIRequestFactory()
        view = StageFieldList.as_view()
        self.request_invalid_value(factory, view, self.stage_field_list_url, {'stage_id': self.invalid_param_alpha})
        self.request_invalid_value(factory, view, self.stage_field_list_url, {'stage_id': self.invalid_param_fullchar})
    ...
    def test_stage_field_list_404(self):
        factory = APIRequestFactory()
        view = StageFieldList.as_view()
        self.create_test_relation_data()
        self.request_not_found(factory, view, self.stage_field_list_url, {'stage_id': self.not_found_id}, 'Field')
