...
class ImportBadgeCommandTests(TestCase):
    ...
    not_exist_badge_url = (
        "https://opedu.lib.osaka-kyoiku.ac.jp/badges/badge_json.php?id=9999"
    )
    ...
    image_url = (
        "https://opedu.lib.osaka-kyoiku.ac.jp/pluginfile.php/1/badges/badgeimage/41/f1"
    )
    image_dir = "/tmp/hoge/"
    ...
    def test_run_ok(self):
        ...
        management.call_command(
            "import_badge",
            url=self.wisdom_badge_url,
            pcid=self.portal_category.id,
            stdout=stdout,
            stderr=stderr,
        )
        self.assertTrue("OK\n" in stdout.getvalue())
    ...
    def test_do_import_notfound_portal_category(self):
        ...
        self.assertEqual(
            e.exception.args[0], "ポータル独自カテゴリに存在しないIDが指定されましたので処理継続できません。pcid: 0"
        )
    ...
    def test_do_import_knowledge_badge(self):
        ...
        with self.assertRaises(AppException) as e:
            cmd.do_import(json, self.portal_category.id)
        self.assertEqual(e.exception.args[0], "スタンプのため処理継続できません。能力バッジを指定してください。")
    ...
    def test_request_get_json_not_exist_badge_url(self):
        ...
        self.assertEqual(
            e.exception.args[0],
            "APIのレスポンスにエラーが含まれるため処理を中断します。エラー内容: ID {$a} のバッジは存在しません。",
        )
    ...
    def make_default_json(self):
        return {
            ...
            "image": {"id": "id3", "author": "author4"},
            "criteria": {"id": "id5", "narrative": "narrative6"},
            "issuer": {
                "name": "name7",
                "url": "url8",
                "email": "email9",
            ...
            "type": "type15",
            "@language": "@language16",
            "alignments": [{"targetName": "targetName17", "targetUrl": "targetUrl18"}],
        }
    ...
    def test_get_badge_data_ok(self):
        ...
        (
            ...
            issuer_url,
            issuer_email,
        ) = cmd.get_badge_data(json)
        self.assert_default_json(
            id,
            name,
            description,
            criteria_narrative,
    ...
    def assert_wisdom_badge(self):
        ...
        self.assertEqual(
            ...
            "https://opedu.lib.osaka-kyoiku.ac.jp/badges/image_author_json.php?id=41",
        )
        ...
        self.assertEqual(
            ...
            "https://opedu.lib.osaka-kyoiku.ac.jp/course/view.php?id=52",
        )
    ...
    def assert_knowledge_badge(self, wisdom_badge):
        ...
        knowledge_badge1 = knowledge_badge_set.get(
            badge_class_id="https://opedu.lib.osaka-kyoiku.ac.jp/badges/badge_json.php?id=36"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            knowledge_badge1.criteria_narrative,
        ...
        self.assertEqual(
            ...
            "https://opedu.lib.osaka-kyoiku.ac.jp/badges/image_author_json.php?id=36",
        )
        ...
        knowledge_badge4 = knowledge_badge_set.get(
            badge_class_id="https://opedu.lib.osaka-kyoiku.ac.jp/badges/badge_json.php?id=39"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "https://opedu.lib.osaka-kyoiku.ac.jp/badges/image_author_json.php?id=39",
        )
        ...
        self.assertEqual(criteria_set.count(), 5)
        criteria2 = criteria_set.get(
            Q(type="ビデオ") & Q(name="2.誰がどのような文脈で、諸外国における授業研究を推進してきたのか？")
        )
        self.assertEqual(criteria2.type, "ビデオ")
        self.assertEqual(criteria2.name, "2.誰がどのような文脈で、諸外国における授業研究を推進してきたのか？")
        criteria4 = criteria_set.get(
            Q(type="ビデオ") & Q(name="4.学び続ける教師を支える、よりよい授業研究のあり方は？")
        )
    ...
    def test_update_issuer_insert(self):
        cmd = import_badge.Command()
        cmd.update_issuer(
            issuer_name="hoge", issuer_url="www.hoge.com", issuer_email="hoge@hoge.com"
        )
    ...
    def test_update_issuer_update(self):
        ...
        cmd.update_issuer(
            issuer_name="hoge",
            issuer_url="www.fugafuga.com",
            issuer_email="hoge@fugafuga.com",
        )
        self.assertEqual(Issuer.objects.all().count(), 1)
        issur = Issuer.objects.all().first()
        self.assertEqual(issur.name, "hoge")
    ...
    def test_create_knowledge_badges(self):
        ...
        wisdom_badge = WisdomBadges.objects.create(
            portal_category=self.portal_category,
            badge_class_id="bid",
            name="n",
            issuer=issuer,
        )
    ...
    def test_create_criterias(self):
        ...
        knowledge_badge = KnowledgeBadges.objects.create(
            wisdom_badges=wisdom_badge, badge_class_id="bid2", name="n2", issuer=issuer
        )
        criteria_list = [
            ["hoge", "ほげほげ"],
            ["fuga", "ふがふが"],
    ...
    def test_create_knowledge_badge(self):
        ...
        json = self.make_default_json()
        knowledge_badge, criteria_list = cmd.create_knowledge_badge(
            json, wisdom_badge, "tmp/hoge.png"
        )
    ...
    def test_parse_narrative_ok_fullcharctor(self):
        cmd = import_badge.Command()
        criteria_list = cmd.parse_narrative(
            "☆☆☆たいぷ☆☆☆ - △△△なまえ△△△\n☆☆☆たいぷ２☆☆☆ - △△△なまえ２△△△\nhoge"
        )
